...
const AuthForm = ({ typeLabel, typeText, activeForm, route }) => {
  ...
  return (
    <div className="text-zinc-800">
      ...
      <div
        className={`${
          route === "signin" && "h-screen items-center"
        } absolute py-14 top-0 left-0 w-screen  flex justify-center   z-20`}
      >
        {/* container */}
        <div className="shadow sm:w-[520px] p-4   bg-white rounded-xl ">
            ...
            {FormNavlinks?.map(({ label, url, id }) => (
              <div
                ...
                className={`py-1 text-center ${
                  activeForm === label ? "border-b-2 border-blue-700" : null
                } cursor-pointer hover:font-bold duration-300 text-sm`}
              >
                <p>
                  {typeText} {label}
                </p>
          ...
          {route === "signin" ? <CustomerLoginForm /> : <CustormerSignupForm />}
        </div>
      </div>
    </div>
  );
